...
int set_coords(void player, float max_alt) {
    if ( getentityproperty(player, "animationid") == openborconstant("ANI_RESPAWN") ) {
        float x,z,a,base;
        int p = getentityproperty(player, "playerindex"); // non puoi ricavarti il player con getentityproperty(getplayerproperty(p, "entity")) poich� non esiste ancora
        int t;
        ...
        if ( get_ready_players_count() > 1 ) {
            if ( check_incam_pos(x,z) == 1 ) { // siamo nel riquadro giusto?
                // se sono nulle allora non � mai stato nel livello il personaggio. Verr� eseguita l'animazione RESPAWN
        ...
        } else { // la funzione incam() ha senso con pi� di un giocatore in gara
                if ( x != NULL() && z != NULL() && a != NULL() && a < max_alt ) {
                    changeentityproperty(player, "position", x, z, a);
                } else {
                    void last_player = getglobalvar("last_player_spawned");
        ...
        // ##### IMPORTANTE: Facciamo eseguire questo script per tutta la durata del respawn!! Perch� al primo frame potrebbero accadere errori!!
    }
}
...
int set_coords_predeath(void player, float max_alt) {
    ...
    if ( refresh_time == 0 && anim_id != openborconstant("ANI_SPAWN") && anim_id != openborconstant("ANI_RESPAWN") ) {
        ...
        if ( ( anim_id == openborconstant("ANI_IDLE") || anim_id == openborconstant("ANI_RUN") || anim_id == openborconstant("ANI_WALK") || anim_id == openborconstant("ANI_LAND")
            ...
            // Memorizziamo l'altitudine cos� da confrontarla con la prossima al prossimo frame. In caso si deve memorizzare la posizione sopra una piattaforma.
...
// Sistemiamo il personaggio dopo il respawn ne caso � dentro una platform (lo script pu� essere eseguito solo ora e non onspawn event)
// 1 ha esito positivo, 0 non esiste l'ultimo player spawned o non � nel quadro, -1 ultimo giocatore spawnato � se stesso, 2 non viene eseguito
        ...
        void last_player = getglobalvar("last_player_spawned");
            ...
            if ( !getentityproperty(last_player,"exists") || getentityproperty(last_player,"type") != openborconstant("type_player") || last_player == NULL() ) {
                ...
                void platform;
                    ...
                    // Respawniamo un valore threshold di px in avanti/dietro/gi�/su rispetto al giocatore gi� in gara
                    for ( i = 0; i <= threshold; ++i ) { // <= cos� pu� assumere valore 0!
